...
params_gridsearch = {
    ...
    'parameters_m' : {'polynomial': [20]}#, 50, 100], 'gaussian': [0.2, 0.5, 1.0], 'uniform': [1.0]}
}
...
for crossover_prob in params_gridsearch['crossover_probabilities']:
    for mutation_prob in params_gridsearch['mutation_probabilities']:
        for crossover in params_gridsearch['crossover']:
            for mutation in params_gridsearch['mutation']:
                for param_m in params_gridsearch['parameters_m'][mutation]:
                    for selection in params_gridsearch['selection']:
                        print('=========================================\nTEST %d\n========================================='%test)
                        folder = 'logs/test'+str(test)
                        mkdir_p(folder)
                        ...
                        params_to_str = json.dumps(params)
                        ...
                        for dim in dimensions:
                            ...
                            for problem_id in problems_to_be_eval:
                                ...
                                nb_gen = max_function_eval//nb_population

                                algo = pg.algorithm(pg.sga( gen       = nb_gen, 
                                                            param_s   = 10,
                                                            **params
                                                        ))
                                all_fitness = []
                                ...
                                plt.plot(all_fitness[best_fitness_idx])
...
for dim in dimensions:
    for problem_id in problems_to_be_eval:
        df = pd.DataFrame.from_csv(folder+'/results-dim' + str(dim) + '.csv', sep = ';')
        max_fes = 10000*dim
